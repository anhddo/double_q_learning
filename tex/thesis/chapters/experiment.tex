...
\begin{figure}[ht!]
    \includegraphics[width=\textwidth]{image/n_chain_fig}
\end{figure}
...
\begin{figure}[ht!]
    ...
    \begin{subfigure}[b]{0.47\textwidth}
        ...
        \label{fig:acrobot}
    \end{subfigure}
    \caption{Cummulative reward on different environments.}
...
\begin{table} 
    ...
    \hline
        \multirow{8}{5em}{Cartpole}&  \multicolumn{3}{|c|}{ \textbf{Observation} }\\ \cline{2-4}
        &\textbf{Name} & \textbf{Min} &  \textbf{Max} \\ \cline{2-4}
    &       Cart Position &             -4.8 &                    4.8 \\
    &       Cart Velocity &             -Inf &                   Inf \\
    &       Pole Angle &                -0.418 rad (-24 deg) &    0.418 rad (24 deg) \\
    &       Pole Angular Velocity  &   -Inf &                    Inf \\\cline{2-4}
        &\multicolumn{3}{|c|} {\textbf{Action}}\\\cline{2-4}
          & \textbf{Value}&   \multicolumn{2}{|l|}{ \textbf{Meaning}}\\\cline{2-4}
          &  0   & \multicolumn{2}{|l|}{Push cart to the left}\\
          &  1   & \multicolumn{2}{|l|}{Push cart to the right}\\
    \hline
        \multirow{8}{3em}{Mountain Cart}&  \multicolumn{3}{|c|}{ \textbf{Observation} }\\ \cline{2-4}
        &\textbf{Name} & \textbf{Min} &  \textbf{Max} \\ \cline{2-4}
    &       Cart Position &             -4.8 &                    4.8 \\
    &       Cart Velocity &             -Inf &                   Inf \\
    &       Pole Angle &                -0.418 rad (-24 deg) &    0.418 rad (24 deg) \\
    ...
    \end{tabular}
\label{table:env}
\caption{Observation and action for OpenAI gym environment}
\end{table}
...
Update the regularized least square estimate as:
  ...
  \] \\
Find the optimistic estimation of Q-function: $\widetilde{Q}_{t}(s, a)=\phi(s, a)^{\top} w_{t}$ \\
  Take action: $a_{t}=\operatorname{argmax}_{a} \widetilde{Q}_{t}\left(s_{t}, a\right)$ and observe $s _{t+1}$ 
  }
 
 \caption{Standard value iteration for average reward problems}
\end{algorithm}
...
Update the regularized least square estimate as:
  ...
  \] \\
Find the optimistic estimation of Q-function: $\widetilde{Q}_{t}(s, a)=\phi(s, a)^{\top} w_{t}$ \\
We uniformly take action at random with probability $\varepsilon $ 
  Take action: $a_{t}=\operatorname{argmax}_{a} \widetilde{Q}_{t}\left(s_{t}, a\right)$ and observe $s _{t+1}$ 
  }
 
 \caption{ $\varepsilon$-greedy value iteration}
\end{algorithm}
