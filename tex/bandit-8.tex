...
$$
    ...
    $\I \{ E _{1}  \cup \E _{2} \} \leq \I \{E _{1}\} + \I \{ E _{2}\}$. We have:
   $$
    \begin{aligned}
...
&\mathbb{E}\left[\sum_{t=1}^{n} \mathbb{I}\left\{\hat{\mu}_{1}(t-1)+\sqrt{\frac{2 \log f(t)}{T_{1}(t-1)}} \leq \mu_{1}-\varepsilon\right\}\right] \\
    &= \sum_{t=1}^{n} \mathbb{P}\left(\hat{\mu}_{1}(t-1)+\sqrt{\frac{2 \log f(t)}{T_{1}(t-1)}} \leq \mu_{1}-\varepsilon\right) \\
\end{aligned}
$$
...
$$
 ...
 $F=\sum_{s=1}^{n} \exp \left(-s \varepsilon^{2} / 2\right)$  
    ...
    . If  $\frac{1}{f(t)} $ is strictly decreasing then $\sum_{k=N+1}^{\infty} \frac{1}{f(t)} \leq \int_{N}^{\infty} \frac{1}{f(t)} \mathrm{dx} \leq \sum_{k=N}^{\infty} \frac{1}{f(t)}
    $ 
   $$
\begin{aligned}
    \sum_{t=1}^{n}\frac{1}{f(t)} &\leq \sum_{t=1}^{2}\frac{1}{f(t)} + \sum_{t=3}^{n}\frac{1}{t log^2 t} \leq \sum_{t=1}^{2}\frac{1}{f(t)} + \int_{2}^{n}\frac{1}{t log^2 t} dt \\
